...
## 2026-06-16 11:08 — Spike-informed review edits
Updated `docs/drafts/market_design.md` in response to review feedback and the temporary Cloudflare reachability spike. The design now records that chromedp-owned launch exposed `navigator.webdriver=true` and looped on Turnstile, while normal visible Chrome launch plus DevTools attach passed Cloudflare and proved direct Go HTTP viable for the tested public `Standard` trade search.
The API guidance now uses an opaque `SessionHandle` instead of passing raw cookies through the core, removes the vague `internal/trade/local` assembly package, adds an explicit `*Client` concurrency contract, adds opt-in metrics, specifies `errors.As`-friendly typed errors, makes fetch batching library-owned, and updates the implementation slice to use direct Go HTTP first with browser-context execution as a fallback.
Validation: `git diff --check` passed, and `moon run docs:build --summary minimal` passed with the existing Material/MkDocs 2.0 warning. Implementation commit: `90c8c02` (`docs: incorporate trade session spike findings`).
