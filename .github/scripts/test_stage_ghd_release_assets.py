...
class StageGhdReleaseAssetsTest(unittest.TestCase):
    def test_stages_expected_assets(self) -> None:
        with fixture() as root:
            ...
            self.assertEqual(
                ...
                [
                    "checksums.txt",
                    "poe2-mcp_1.2.3_darwin_amd64",
                    "poe2-mcp_1.2.3_darwin_amd64.sbom.json",
                    "poe2-mcp_1.2.3_darwin_arm64",
                    "poe2-mcp_1.2.3_darwin_arm64.sbom.json",
                    "poe2-mcp_1.2.3_linux_amd64",
                    "poe2-mcp_1.2.3_linux_amd64.sbom.json",
                    "poe2-mcp_1.2.3_linux_arm64",
                    "poe2-mcp_1.2.3_linux_arm64.sbom.json",
                ],
            )
            linux_binary = root / "dist/release-assets/poe2-mcp_1.2.3_linux_amd64"
            mode = linux_binary.stat().st_mode
            self.assertTrue(mode & stat.S_IXUSR)
            self.assertIn("dist/release-assets/poe2-mcp_1.2.3_linux_arm64", stdout)

    def test_fails_on_missing_checksum_entry(self) -> None:
        with fixture(missing_checksum="poe2-mcp_1.2.3_linux_arm64") as root:
            result, _, stderr = run_script(root)

            self.assertEqual(result, 1)
            self.assertIn("missing checksum entry", stderr)
            self.assertIn("poe2-mcp_1.2.3_linux_arm64", stderr)

    def test_fails_on_checksum_mismatch(self) -> None:
        override = ("poe2-mcp_1.2.3_linux_amd64", "0" * 64)
        with fixture(checksum_override=override) as root:
            result, _, stderr = run_script(root)

            self.assertEqual(result, 1)
            self.assertIn("checksum mismatch for poe2-mcp_1.2.3_linux_amd64", stderr)
    ...
    def test_fails_on_missing_os_arch_asset(self) -> None:
        with fixture(omit_artifact=("linux", "arm64", "Binary")) as root:
            ...
            self.assertEqual(result, 1)
            self.assertIn("missing expected binary asset poe2-mcp_1.2.3_linux_arm64", stderr)
...
def run_script(root: Path) -> tuple[int, str, str]:
    stdout = io.StringIO()
    stderr = io.StringIO()
    with working_directory(root), github_repository("jmgilman/poe"):
...
def fixture(
    *,
    signer: str = "jmgilman/poe/.github/workflows/release.yml",
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        for goos, goarch in PLATFORMS:
            binary_name = f"poe2-mcp_1.2.3_{goos}_{goarch}"
        ...
        if extra_binary:
            extra_name = "poe2-mcp_1.2.3_freebsd_amd64"
...
[[packages]]
name = "poe2-mcp"
description = "Path of Exile 2 MCP server starter CLI."
tag_pattern = "v${{version}}"

[[packages.assets]]
os = "darwin"
arch = "amd64"
pattern = "poe2-mcp_${{version}}_darwin_amd64"

[[packages.assets]]
os = "darwin"
arch = "arm64"
pattern = "poe2-mcp_${{version}}_darwin_arm64"

[[packages.assets]]
os = "linux"
arch = "amd64"
pattern = "poe2-mcp_${{version}}_linux_amd64"

[[packages.assets]]
os = "linux"
arch = "arm64"
pattern = "poe2-mcp_${{version}}_linux_arm64"
